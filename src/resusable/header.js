...
export default function PrimarySearchAppBar(props) {
  ...
  const [headerList, setHeaderList] = useState(headerLists);
          ...
          {headerList.map((ele) => (
            <ListItem
              ...
              onClick={() =>
                setHeaderList((l1) =>
                  l1.map((list) => {
                    if (list.label === ele.label) list.popup = !list.popup;
                    return list;
                  })
                )
              }
      ...
      <Grid container spacing={2}>
        ...
        <Grid item style={{ flex: 1 }}>
          <TextField
            ...
            style={{ border: 0 }}
            placeholder="Search for watercolor, script fonts, procreate"
            value={search}
  ...
  const desktopList = (
    <Grid container>
      {headerList.map((ele, ind) => (
        ...
        >
          <Typography className={classes.desktopItem}>{ele.label}</Typography>
        </Grid>
      ))}
    </Grid>
  );
